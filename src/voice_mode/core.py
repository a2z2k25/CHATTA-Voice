...
async def play_chime_start(
    ...
    try:
        ...
        # Check if Bluetooth device (based on leading_silence request)
        # Use Bluetooth version if leading_silence is >= 0.8 seconds
        if leading_silence and leading_silence >= 0.8:
            audio_file = audio_dir / "start_chime_bluetooth.wav"
        else:
            audio_file = audio_dir / "start_chime.wav"
        
        # Check if file exists
        if not audio_file.exists():
            # Fallback to programmatic generation if files don't exist
            import sounddevice as sd
            ...
            # Play pre-generated file using system audio player
            if sys.platform == 'darwin':  # macOS
                # Use afplay - native macOS audio player
                subprocess.run(['afplay', str(audio_file)], check=True, capture_output=True)
            elif sys.platform.startswith('linux'):
                # Try aplay or paplay for Linux
                try:
                ...
                # Windows or fallback - try simpleaudio with error handling
                try:
                    ...
                    # If simpleaudio not available, use sounddevice
                    import sounddevice as sd
                    chime = generate_chime(
                        [800, 1000], 
                        duration=0.1, 
...
async def play_chime_end(
    ...
    try:
        ...
        # Use Bluetooth version if leading_silence is >= 0.8 seconds
        if leading_silence and leading_silence >= 0.8:
            audio_file = audio_dir / "end_chime_bluetooth.wav"
        else:
            audio_file = audio_dir / "end_chime.wav"
