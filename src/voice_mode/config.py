"""
Configuration and shared utilities for CHATTA Server.

This module contains all configuration constants, global state, initialization functions,
and shared utilities used across the CHATTA server.
"""
...
def load_chatta_env():
    """Load configuration from chatta.env file if it exists, creating a default if not."""
    # Try new filename first
    config_path = Path.home() / ".chatta" / "chatta.env"
    
    # Backwards compatibility: check for old voicemode filenames if new doesn't exist
    if not config_path.exists():
        old_path = Path.home() / ".voicemode" / "voicemode.env"
        if old_path.exists():
            config_path = old_path
            print(f"Warning: Using deprecated voicemode.env - please rename to chatta.env and move to ~/.chatta/")
    
    if not config_path.exists():
        # Create default template
        config_path.parent.mkdir(parents=True, exist_ok=True)
        default_config = '''# CHATTA Configuration File
...
# Base directory for all CHATTA data (default: ~/.chatta)
# CHATTA_BASE_DIR=~/.chatta
...
# CHATTA_SAVE_AUDIO=false

# Save transcription files (true/false)
# CHATTA_SAVE_TRANSCRIPTIONS=false
...
# CHATTA_TTS_BASE_URLS=http://127.0.0.1:8880/v1,https://api.openai.com/v1

# Comma-separated list of STT endpoints
# CHATTA_STT_BASE_URLS=http://127.0.0.1:2022/v1,https://api.openai.com/v1

# Comma-separated list of preferred voices
# CHATTA_TTS_VOICES=af_sky,alloy

# Comma-separated list of preferred models
# CHATTA_TTS_MODELS=tts-1,tts-1-hd,gpt-4o-mini-tts

# Prefer local providers over cloud (true/false)
# CHATTA_PREFER_LOCAL=true

# Always attempt local providers (true/false)
# CHATTA_ALWAYS_TRY_LOCAL=true

# Auto-start Kokoro service (true/false)
# CHATTA_AUTO_START_KOKORO=false
...
# CHATTA_WHISPER_MODEL=large-v2

# Whisper server port (default: 2022)
# CHATTA_WHISPER_PORT=2022

# Language for transcription (auto, en, es, fr, de, it, pt, ru, zh, ja, ko, etc.)
# CHATTA_WHISPER_LANGUAGE=auto

# Path to Whisper models
# CHATTA_WHISPER_MODEL_PATH=~/.chatta/models/whisper
...
# CHATTA_KOKORO_PORT=8880

# Directory for Kokoro models
# CHATTA_KOKORO_MODELS_DIR=~/.chatta/models/kokoro

# Directory for Kokoro cache
# CHATTA_KOKORO_CACHE_DIR=~/.chatta/cache/kokoro

# Default Kokoro voice
# CHATTA_KOKORO_DEFAULT_VOICE=af_sky
...
BASE_DIR = expand_path(os.getenv("CHATTA_BASE_DIR", str(Path.home() / ".chatta")))
...
MODELS_DIR = expand_path(os.getenv("CHATTA_MODELS_DIR", str(BASE_DIR / "models")))

# Debug configuration
DEBUG = os.getenv("CHATTA_DEBUG", "").lower() in ("true", "1", "yes", "on")
TRACE_DEBUG = os.getenv("CHATTA_DEBUG", "").lower() == "trace"
VAD_DEBUG = os.getenv("CHATTA_VAD_DEBUG", "").lower() in ("true", "1", "yes", "on")
DEBUG_DIR = LOGS_DIR / "debug"  # Debug files now go under logs

# Master save-all configuration
SAVE_ALL = os.getenv("CHATTA_SAVE_ALL", "").lower() in ("true", "1", "yes", "on")

# Audio saving configuration
# Enable if SAVE_ALL is true, DEBUG is true, or individually enabled
SAVE_AUDIO = SAVE_ALL or DEBUG or os.getenv("CHATTA_SAVE_AUDIO", "").lower() in ("true", "1", "yes", "on")
SAVE_TRANSCRIPTIONS = SAVE_ALL or DEBUG or os.getenv("CHATTA_SAVE_TRANSCRIPTIONS", "").lower() in ("true", "1", "yes", "on")

# Audio feedback configuration
AUDIO_FEEDBACK_ENABLED = os.getenv("CHATTA_AUDIO_FEEDBACK", "true").lower() in ("true", "1", "yes", "on")

# Skip TTS configuration (skip text-to-speech for faster responses)
SKIP_TTS = os.getenv("CHATTA_SKIP_TTS", "false").lower() in ("true", "1", "yes", "on")

# Local provider preference configuration
PREFER_LOCAL = os.getenv("CHATTA_PREFER_LOCAL", "true").lower() in ("true", "1", "yes", "on")

# Always try local providers (don't mark them as permanently unavailable)
ALWAYS_TRY_LOCAL = os.getenv("CHATTA_ALWAYS_TRY_LOCAL", "true").lower() in ("true", "1", "yes", "on")

# Use simple failover without health checks
SIMPLE_FAILOVER = os.getenv("CHATTA_SIMPLE_FAILOVER", "true").lower() in ("true", "1", "yes", "on")

# Auto-start configuration
AUTO_START_KOKORO = os.getenv("CHATTA_AUTO_START_KOKORO", "").lower() in ("true", "1", "yes", "on")
...
TTS_BASE_URLS = parse_comma_list("CHATTA_TTS_BASE_URLS", "http://127.0.0.1:8880/v1,https://api.openai.com/v1")
STT_BASE_URLS = parse_comma_list("CHATTA_STT_BASE_URLS", "http://127.0.0.1:2022/v1,https://api.openai.com/v1")
TTS_VOICES = parse_comma_list("CHATTA_TTS_VOICES", "af_sky,alloy")
TTS_MODELS = parse_comma_list("CHATTA_TTS_MODELS", "tts-1,tts-1-hd,gpt-4o-mini-tts")

# Legacy variables have been removed - use the new list-based configuration:
# - CHATTA_TTS_BASE_URLS (comma-separated list)
# - CHATTA_STT_BASE_URLS (comma-separated list)
# - CHATTA_TTS_VOICES (comma-separated list)
# - CHATTA_TTS_MODELS (comma-separated list)
...
WHISPER_MODEL = os.getenv("CHATTA_WHISPER_MODEL", "base")
WHISPER_PORT = int(os.getenv("CHATTA_WHISPER_PORT", "2022"))
WHISPER_LANGUAGE = os.getenv("CHATTA_WHISPER_LANGUAGE", "auto")
WHISPER_MODEL_PATH = expand_path(os.getenv("CHATTA_WHISPER_MODEL_PATH", str(Path.home() / ".chatta" / "services" / "whisper" / "models")))

# ==================== KOKORO CONFIGURATION ====================

# Kokoro-specific configuration
KOKORO_PORT = int(os.getenv("CHATTA_KOKORO_PORT", "8880"))
KOKORO_MODELS_DIR = expand_path(os.getenv("CHATTA_KOKORO_MODELS_DIR", str(BASE_DIR / "models" / "kokoro")))
KOKORO_CACHE_DIR = expand_path(os.getenv("CHATTA_KOKORO_CACHE_DIR", str(BASE_DIR / "cache" / "kokoro")))
KOKORO_DEFAULT_VOICE = os.getenv("CHATTA_KOKORO_DEFAULT_VOICE", "af_sky")

# ==================== LIVEKIT CONFIGURATION ====================

# LiveKit-specific configuration
LIVEKIT_PORT = int(os.getenv("CHATTA_LIVEKIT_PORT", "7880"))
LIVEKIT_URL = os.getenv("LIVEKIT_URL", f"ws://localhost:{LIVEKIT_PORT}")
LIVEKIT_API_KEY = os.getenv("LIVEKIT_API_KEY", "devkey")
LIVEKIT_API_SECRET = os.getenv("LIVEKIT_API_SECRET", "secret")

# LiveKit Frontend configuration
FRONTEND_HOST = os.getenv("CHATTA_FRONTEND_HOST", "127.0.0.1")
FRONTEND_PORT = int(os.getenv("CHATTA_FRONTEND_PORT", "3000"))

# ==================== SERVICE MANAGEMENT CONFIGURATION ====================

# Auto-enable services after installation
SERVICE_AUTO_ENABLE = env_bool("CHATTA_SERVICE_AUTO_ENABLE", False)
...
DISABLE_SILENCE_DETECTION = os.getenv("CHATTA_DISABLE_SILENCE_DETECTION", "false").lower() in ("true", "1", "yes", "on")

# VAD (Voice Activity Detection) configuration
VAD_AGGRESSIVENESS = int(os.getenv("CHATTA_VAD_AGGRESSIVENESS", "2"))  # 0-3, higher = more aggressive
SILENCE_THRESHOLD_MS = int(os.getenv("CHATTA_SILENCE_THRESHOLD_MS", "1000"))  # Stop after 1000ms (1 second) of silence
MIN_RECORDING_DURATION = float(os.getenv("CHATTA_MIN_RECORDING_DURATION", "0.5"))  # Minimum 0.5s recording
VAD_CHUNK_DURATION_MS = 30  # VAD frame size (must be 10, 20, or 30ms)
INITIAL_SILENCE_GRACE_PERIOD = float(os.getenv("CHATTA_INITIAL_SILENCE_GRACE_PERIOD", "1"))  # No initial silence grace period by default

# Default listen duration for converse tool
DEFAULT_LISTEN_DURATION = float(os.getenv("CHATTA_DEFAULT_LISTEN_DURATION", "120.0"))  # Default 120s listening time

# Audio feedback pip/chime configuration
# Leading silence before pips to allow Bluetooth devices to wake up
PIP_LEADING_SILENCE = float(os.getenv("CHATTA_PIP_LEADING_SILENCE", "0.1"))  # Default 0.1s - minimal delay for Bluetooth
# Trailing silence after pips to prevent cutoff
PIP_TRAILING_SILENCE = float(os.getenv("CHATTA_PIP_TRAILING_SILENCE", "0.2"))  # Default 0.2s - reduced for responsiveness

# Audio format configuration
AUDIO_FORMAT = os.getenv("CHATTA_AUDIO_FORMAT", "pcm").lower()
TTS_AUDIO_FORMAT = os.getenv("CHATTA_TTS_AUDIO_FORMAT", "pcm").lower()  # Default to PCM for optimal streaming
# STT requires a format supported by the STT provider - PCM is not supported by OpenAI Whisper
STT_AUDIO_FORMAT = os.getenv("CHATTA_STT_AUDIO_FORMAT", "mp3" if AUDIO_FORMAT == "pcm" else AUDIO_FORMAT).lower()
...
OPUS_BITRATE = int(os.getenv("CHATTA_OPUS_BITRATE", "32000"))  # Default 32kbps for voice
MP3_BITRATE = os.getenv("CHATTA_MP3_BITRATE", "64k")  # Default 64kbps
AAC_BITRATE = os.getenv("CHATTA_AAC_BITRATE", "64k")  # Default 64kbps

# ==================== STREAMING CONFIGURATION ====================

# Streaming playback configuration
STREAMING_ENABLED = os.getenv("CHATTA_STREAMING_ENABLED", "true").lower() in ("true", "1", "yes", "on")
STREAM_CHUNK_SIZE = int(os.getenv("CHATTA_STREAM_CHUNK_SIZE", "4096"))  # Download chunk size
STREAM_BUFFER_MS = int(os.getenv("CHATTA_STREAM_BUFFER_MS", "150"))  # Initial buffer before playback
STREAM_MAX_BUFFER = float(os.getenv("CHATTA_STREAM_MAX_BUFFER", "2.0"))  # Max buffer in seconds
...
def setup_logging() -> logging.Logger:
    """Configure logging for the CHATTA server.
    
    Returns:
        Logger instance configured for CHATTA
    """
