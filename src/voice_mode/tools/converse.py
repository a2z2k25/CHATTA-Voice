...
# Import the shared mcp instance properly to avoid circular import
from voice_mode import mcp
        ...
        if not wait_for_response:
            try:
                async with audio_operation_lock:
                    if should_skip_tts or not message.strip():
                        # Skip TTS entirely or when message is empty
                        success = True
                        tts_metrics = {
                            'ttfa': 0,
                            'generation': 0,
        ...
        if transport == "livekit":
            ...
            try:
                async with audio_operation_lock:
                    ...
                    if should_skip_tts or not message.strip():
                        # Skip TTS entirely for faster response or when message is empty
                        tts_success = True
                        tts_metrics = {
                            'ttfa': 0,
                            'generation': 0,
